...
' Version: 1.5 - Complete error suppression mode
...
Function GetListerKey(lister)
    ...
    If Not lister Is Nothing And Not lister.ActiveTab Is Nothing Then
...
Function IsListerValid(lister)
    On Error Resume Next
    ' Safely check if a lister object is valid and accessible
    IsListerValid = False
...
Function SafeGetDualMode(lister)
    On Error Resume Next
    ' Safely retrieve the dual mode, returning empty string on error
    SafeGetDualMode = ""
    
    If Not IsListerValid(lister) Then
        Err.Clear
        Exit Function
    End If
    
    SafeGetDualMode = GetDisplayModeType(lister.dual)
...
Sub ManageToolbarsForMode(lister, newMode, oldMode)
    ...
    ' Verify lister is still valid before proceeding
    If Not IsListerValid(lister) Then
        DebugOutput 3, "Lister no longer valid in ManageToolbarsForMode, skipping"
        Err.Clear
        Exit Sub
    End If
...
Function OnListerUIChange(listerUIChangeData)
    ...
    ' Use safe validation function
    If Not IsListerValid(lister) Then
        DebugOutput 3, "Lister is not valid or is closing, skipping"
        Err.Clear
        Exit Function
    End If
    
    ' Use safe getter for dual mode
    Dim currentMode
    currentMode = SafeGetDualMode(lister)
    
    If currentMode = "" Then
        DebugOutput 3, "Unable to get current display mode (lister may be closing), skipping"
        Err.Clear
        Exit Function
    End If
...
Function OnActivateLister(activateListerData)
    ...
    ' Use safe validation function
    If Not IsListerValid(lister) Then
        DebugOutput 3, "Lister is not valid or is initializing, skipping"
        Err.Clear
        Exit Function
    End If
    ...
    If currentMode = "" Then
        DebugOutput 3, "Unable to get current display mode (lister may be initializing), skipping"
        Err.Clear
        Exit Function
    End If
